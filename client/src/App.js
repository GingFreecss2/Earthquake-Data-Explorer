    ...
    <div className='app'>
      <nav>
        <ul className='nav-bar'>
          <li style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50px', background: 'rgb(28, 28, 28)', color: 'white', fontSize: '2.5em' }} >
            <WiEarthquake />Global Earthquake Watch
          </li>
        </ul>
      </nav>
      <p className='directions' style={{ textAlign: 'center', fontSize: '1.5em', color: 'black', fontWeight: 'bold' }} >
        {' '}
        Explore Earthquake Data from the Last 30 Days using the following criteria:
      </p>
      <div className='main'>
        <div className='type-selector' style={{ display: 'block', margin: '0 auto', textAlign: 'center' }} >
          <ul>
            <li >
              <select
                name='types'
        ...
        {documents && (
          <div className='search-results'  >
            {documents.length > 0 ? (
              <p style={{ fontSize: '1.8em', color: 'black', fontWeight: 'bold' }} > Number of hits: {documents.length}</p>
            ) : (
              <p style={{ fontSize: '1.8em', color: 'black', fontWeight: 'bold' }}> No results found. Try broadening your search criteria.</p>
            )}
            {documents.map((document) => (
              <div className='results-card' style={{ backgroundColor: 'rgb(25, 129, 67, 0.7)', maxWidth: 'calc(33.33% - 20px)', margin: '1%', padding: '10px', borderRadius: '10px', fontWeight: 'bold', display: 'inline-block', width: '28%' }}>
